...
def update_graphs(vehicle_type, selected_year):
    # Filter data based on selected values
    filtered_data = df[(df['VehicleType'] == vehicle_type) & (df['Year'] == selected_year)]

    # Create Sales graph
    sales_fig = px.bar(filtered_data, x='Year', y='Sales', title=f'Sales for {vehicle_type} in {selected_year}')

    # Create Average Price graph
    avg_price_fig = px.line(filtered_data, x='Year', y='AveragePrice', title=f'Average Price for {vehicle_type} in {selected_year}')
